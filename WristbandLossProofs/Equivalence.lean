...
3. set `Z = √S • U`.
-/
def sphericalLaw (d : ℕ) (hDim : 1 ≤ d) (radiusSqLaw : Distribution NNReal) : Distribution (Vec d) :=
  pushforward (radialReconstruct d) (productLaw radiusSqLaw (sphereUniform d hDim))
...
theorem sphericalLaw_rotationInvariant
    ...
    IsRotationInvariant d (sphericalLaw d hDim1 radiusSqLaw) := by
  intro O
  ...
  have hProdMap :
      Measure.map
          ...
          ((radiusSqLaw : Measure NNReal).prod (sphereUniform d hDim1 : Measure (Sphere d))) =
        ((radiusSqLaw : Measure NNReal).prod (sphereUniform d hDim1 : Measure (Sphere d))) := by
    calc
      Measure.map
          (Prod.map id (rotateSphere O))
          ((radiusSqLaw : Measure NNReal).prod (sphereUniform d hDim1 : Measure (Sphere d)))
      ...
      _ = (radiusSqLaw : Measure NNReal).prod
            (Measure.map (rotateSphere O) (sphereUniform d hDim1 : Measure (Sphere d))) := by
            simp [Measure.map_id]
      _ = (radiusSqLaw : Measure NNReal).prod (sphereUniform d hDim1 : Measure (Sphere d)) := by
            have hRot :
...
theorem sphericalLaw_determinedByRadius
    ...
    (hU : pushforward U μ hUmeas = sphereUniform d hDim1)
    (hIndep : IndepLaw μ S U hS hUmeas) :
    pushforward (fun ω => (Real.sqrt (S ω : ℝ)) • (U ω).1) μ hReconstruct =
      sphericalLaw d hDim1 (pushforward S μ hS) := by
  have _ : 2 ≤ d := hDim
  ...
  calc
    ...
    _ = pushforward (radialReconstruct d)
          ...
          (measurable_radialReconstruct d) := by
            rw [hIndep]
    _ = pushforward (radialReconstruct d)
          (productLaw (pushforward S μ hS) (sphereUniform d hDim1))
          (measurable_radialReconstruct d) := by
            simp [hU]
    _ = sphericalLaw d hDim1 (pushforward S μ hS) := by
            rfl
...
lemma indepLaw_direction_radius_of_wristbandUniform
    ...
    (hUniform : wristbandLaw d Q = wristbandUniform d hDim1) :
  ...
  have hDir : pushforward (direction (d := d)) Q hDirMeas = sphereUniform d hDim1 := by
    have hDirFromUniform :
        pushforward Prod.fst (wristbandLaw d Q) measurable_fst = sphereUniform d hDim1 := by
      calc
        pushforward Prod.fst (wristbandLaw d Q) measurable_fst
            = pushforward Prod.fst (wristbandUniform d hDim1) measurable_fst := by
                rw [hUniform]
        _ = sphereUniform d hDim1 := by
              apply Subtype.ext
              change
                Measure.map Prod.fst ((wristbandUniform d hDim1 : Distribution (Wristband d)) : Measure (Wristband d))
                  = (sphereUniform d hDim1 : Measure (Sphere d))
    ...
    calc
      pushforward (direction (d := d)) Q hDirMeas
          = pushforward Prod.fst (wristbandLaw d Q) measurable_fst := by
              ...
              rfl
      _ = sphereUniform d hDim1 := hDirFromUniform
    ...
    have hTMarginal :
      ...
      calc
        pushforward Prod.snd (wristbandLaw d Q) measurable_snd
            = pushforward Prod.snd (wristbandUniform d hDim1) measurable_snd := by
                rw [hUniform]
        _ = uniform01 := by
              apply Subtype.ext
              change
                Measure.map Prod.snd ((wristbandUniform d hDim1 : Distribution (Wristband d)) : Measure (Wristband d))
  ...
  have hIicRect :
    ...
    have hSphereFactor :
        (sphereUniform d hDim1 : Measure (Sphere d)) s
    ...
    calc
      ...
      _ = ((wristbandUniform d hDim1 : Distribution (Wristband d)) : Measure (Wristband d))
            (s ×ˢ Set.Iic (chiSqCDFToUnit d x)) := by
              simpa [hUniform]
      _ = (sphereUniform d hDim1 : Measure (Sphere d)) s *
            ((uniform01 : Distribution UnitInterval) : Measure UnitInterval)
              (Set.Iic (chiSqCDFToUnit d x)) := by
...
theorem wristbandEquivalence_forward
    ...
    (hDim : 2 ≤ d)
    (hDim1 : 1 ≤ d := le_trans (by decide : 1 ≤ 2) hDim) :
    wristbandLaw d (gaussianNZ d hDim1) = wristbandUniform d hDim1 := by
  ...
  have hDir :
      pushforward (direction (d := d)) (gaussianNZ d hDim1) hDirMeas = sphereUniform d hDim1 := by
    simpa [hDirMeas] using gaussianPolar_direction_uniform d hDim1
  have hRad : pushforward (radiusSq (d := d)) (gaussianNZ d hDim1) hRadMeas = chiSqRadiusLaw d := by
    simpa [hRadMeas] using gaussianPolar_radius_chiSq d hDim1

  have hJoint :
      pushforward (fun z : VecNZ d => (direction (d := d) z, radiusSq (d := d) z))
        (gaussianNZ d hDim1) hPairMeas
      = productLaw
          (pushforward (direction (d := d)) (gaussianNZ d hDim1) hDirMeas)
          (pushforward (radiusSq (d := d)) (gaussianNZ d hDim1) hRadMeas) :=
    gaussianPolar_independent d hDim1
  ...
  have hMapProd :
    ...
    change
      Measure.map
          ...
          (((pushforward (direction (d := d)) (gaussianNZ d hDim1) hDirMeas : Distribution (Sphere d)) : Measure (Sphere d)).prod
            ((pushforward (radiusSq (d := d)) (gaussianNZ d hDim1) hRadMeas : Distribution NNReal) : Measure NNReal))
      =
      (((pushforward (direction (d := d)) (gaussianNZ d hDim1) hDirMeas : Distribution (Sphere d)) : Measure (Sphere d)).prod
        ((pushforward (chiSqCDFToUnit d)
          (pushforward (radiusSq (d := d)) (gaussianNZ d hDim1) hRadMeas) hCDFMeas : Distribution UnitInterval) : Measure UnitInterval))
    calc
      Measure.map
          (Prod.map id (chiSqCDFToUnit d))
          (((pushforward (direction (d := d)) (gaussianNZ d hDim1) hDirMeas : Distribution (Sphere d)) : Measure (Sphere d)).prod
            ((pushforward (radiusSq (d := d)) (gaussianNZ d hDim1) hRadMeas : Distribution NNReal) : Measure NNReal))
          = (Measure.map id ((pushforward (direction (d := d)) (gaussianNZ d hDim1) hDirMeas : Distribution (Sphere d)) : Measure (Sphere d))).prod
              (Measure.map (chiSqCDFToUnit d) ((pushforward (radiusSq (d := d)) (gaussianNZ d hDim1) hRadMeas : Distribution NNReal) : Measure NNReal)) := by
              symm
              exact Measure.map_prod_map
                (((pushforward (direction (d := d)) (gaussianNZ d hDim1) hDirMeas : Distribution (Sphere d)) : Measure (Sphere d)))
                (((pushforward (radiusSq (d := d)) (gaussianNZ d hDim1) hRadMeas : Distribution NNReal) : Measure NNReal))
                measurable_id hCDFMeas
      _ =
        (((pushforward (direction (d := d)) (gaussianNZ d hDim1) hDirMeas : Distribution (Sphere d)) : Measure (Sphere d)).prod
          (Measure.map (chiSqCDFToUnit d)
            ((pushforward (radiusSq (d := d)) (gaussianNZ d hDim1) hRadMeas : Distribution NNReal) : Measure NNReal))) := by
            simp [Measure.map_id]
      _ =
        (((pushforward (direction (d := d)) (gaussianNZ d hDim1) hDirMeas : Distribution (Sphere d)) : Measure (Sphere d)).prod
          ((pushforward (chiSqCDFToUnit d)
  ...
  have hWristbandAsMap :
      wristbandLaw d (gaussianNZ d hDim1)
      = pushforward (Prod.map id (chiSqCDFToUnit d))
          (pushforward (fun z : VecNZ d => (direction (d := d) z, radiusSq (d := d) z))
            (gaussianNZ d hDim1) hPairMeas)
          hProdMapMeas := by
    apply Subtype.ext
    change
      Measure.map (wristbandMap d) (gaussianNZ d hDim1 : Measure (VecNZ d))
      =
      Measure.map
        (Prod.map id (chiSqCDFToUnit d))
        (Measure.map (fun z : VecNZ d => (direction (d := d) z, radiusSq (d := d) z))
          (gaussianNZ d hDim1 : Measure (VecNZ d)))
    calc
      Measure.map (wristbandMap d) (gaussianNZ d hDim1 : Measure (VecNZ d))
          = Measure.map
              ((Prod.map id (chiSqCDFToUnit d)) ∘
                (fun z : VecNZ d => (direction (d := d) z, radiusSq (d := d) z)))
              (gaussianNZ d hDim1 : Measure (VecNZ d)) := by
                rfl
      _ = Measure.map
            (Prod.map id (chiSqCDFToUnit d))
            (Measure.map (fun z : VecNZ d => (direction (d := d) z, radiusSq (d := d) z))
  ...
  calc
    ...
    _ = productLaw
          ...
          (pushforward (chiSqCDFToUnit d)
            (pushforward (radiusSq (d := d)) (gaussianNZ d hDim1) hRadMeas)
            hCDFMeas) := hMapProd
    _ = productLaw (sphereUniform d hDim1)
          (pushforward (chiSqCDFToUnit d) (chiSqRadiusLaw d) hCDFMeas) := by
          simp [hDir, hRad]
    _ = productLaw (sphereUniform d hDim1) uniform01 := by
          simp [hPIT]
    _ = wristbandUniform d hDim1 := by
          rfl
...
theorem wristbandEquivalence_backward
  ...
  have hDirQ : pushforward (direction (d := d)) Q hDirMeas = sphereUniform d hDim1 := by
    have hDirFromUniform :
        pushforward Prod.fst (wristbandLaw d Q) measurable_fst = sphereUniform d hDim1 := by
      calc
        pushforward Prod.fst (wristbandLaw d Q) measurable_fst
  ...
  have hIndepQ :
    ...
    exact indepLaw_direction_radius_of_wristbandUniform d hDim hDim1 Q hUniform

  have hDirG :
      pushforward (direction (d := d)) (gaussianNZ d hDim1) hDirMeas = sphereUniform d hDim1 := by
    simpa [hDirMeas] using gaussianPolar_direction_uniform d hDim1
  have hRadG : pushforward (radiusSq (d := d)) (gaussianNZ d hDim1) hRadMeas = chiSqRadiusLaw d := by
    simpa [hRadMeas] using gaussianPolar_radius_chiSq d hDim1
  have hIndepG :
      IndepLaw (gaussianNZ d hDim1) (direction (d := d)) (radiusSq (d := d)) hDirMeas hRadMeas := by
    simpa [hDirMeas, hRadMeas] using gaussianPolar_independent d hDim1
  ...
  have hIndepGSwap :
      IndepLaw (gaussianNZ d hDim1) (radiusSq (d := d)) (direction (d := d)) hRadMeas hDirMeas := by
    unfold IndepLaw at hIndepG ⊢
    calc
      pushforward (fun ω : VecNZ d => (radiusSq (d := d) ω, direction (d := d) ω))
          (gaussianNZ d hDim1) (hRadMeas.prodMk hDirMeas)
          = pushforward Prod.swap
              (pushforward (fun ω : VecNZ d => (direction (d := d) ω, radiusSq (d := d) ω))
                (gaussianNZ d hDim1) (hDirMeas.prodMk hRadMeas))
              measurable_swap := by
                apply Subtype.ext
                change
                ...
                rfl
      _ = pushforward Prod.swap
            (productLaw (pushforward (direction (d := d)) (gaussianNZ d hDim1) hDirMeas)
              (pushforward (radiusSq (d := d)) (gaussianNZ d hDim1) hRadMeas))
            measurable_swap := by
              rw [hIndepG]
      _ = productLaw (pushforward (radiusSq (d := d)) (gaussianNZ d hDim1) hRadMeas)
            (pushforward (direction (d := d)) (gaussianNZ d hDim1) hDirMeas) := by
              apply Subtype.ext
              change
                Measure.map Prod.swap
                  (((pushforward (direction (d := d)) (gaussianNZ d hDim1) hDirMeas : Distribution (Sphere d)) :
                    Measure (Sphere d)).prod
                    ((pushforward (radiusSq (d := d)) (gaussianNZ d hDim1) hRadMeas : Distribution NNReal) :
                      Measure NNReal))
                =
                (((pushforward (radiusSq (d := d)) (gaussianNZ d hDim1) hRadMeas : Distribution NNReal) :
                  Measure NNReal).prod
              ...
              simpa using
                (Measure.prod_swap
                  (μ := (((pushforward (direction (d := d)) (gaussianNZ d hDim1) hDirMeas : Distribution (Sphere d)) :
                    Measure (Sphere d))))
                  (ν := (((pushforward (radiusSq (d := d)) (gaussianNZ d hDim1) hRadMeas : Distribution NNReal) :
                    Measure NNReal))))
  ...
  have hPushQ :
      pushforward (Subtype.val : VecNZ d → Vec d) Q measurable_subtype_coe
        = sphericalLaw d hDim1 (pushforward (radiusSq (d := d)) Q hRadMeas) := by
    calc
      pushforward (Subtype.val : VecNZ d → Vec d) Q measurable_subtype_coe
                ...
                change
                  Measure.map (Subtype.val : VecNZ d → Vec d) (Q : Measure (VecNZ d))
                    = Measure.map
                        ...
                        (Q : Measure (VecNZ d))
                rw [hReconstructEq]
      _ = sphericalLaw d hDim1 (pushforward (radiusSq (d := d)) Q hRadMeas) := by
            exact sphericalLaw_determinedByRadius d hDim hDim1 Q
              (radiusSq (d := d))
              (direction (d := d))
              hRadMeas
              hDirMeas
  ...
  have hPushG :
      pushforward (Subtype.val : VecNZ d → Vec d) (gaussianNZ d hDim1) measurable_subtype_coe
        = sphericalLaw d hDim1 (chiSqRadiusLaw d) := by
    calc
      pushforward (Subtype.val : VecNZ d → Vec d) (gaussianNZ d hDim1) measurable_subtype_coe
          = pushforward (fun z : VecNZ d => (Real.sqrt (radiusSq (d := d) z : ℝ)) • (direction (d := d) z).1)
              (gaussianNZ d hDim1) hReconstructMeas := by
                apply Subtype.ext
                change
                  Measure.map (Subtype.val : VecNZ d → Vec d) (gaussianNZ d hDim1 : Measure (VecNZ d))
                    = Measure.map
                        (fun z : VecNZ d => (Real.sqrt (radiusSq (d := d) z : ℝ)) • (direction (d := d) z).1)
                        (gaussianNZ d hDim1 : Measure (VecNZ d))
                rw [hReconstructEq]
      _ = sphericalLaw d hDim1 (pushforward (radiusSq (d := d)) (gaussianNZ d hDim1) hRadMeas) := by
            exact sphericalLaw_determinedByRadius d hDim hDim1 (gaussianNZ d hDim1)
              (radiusSq (d := d))
              (direction (d := d))
              hRadMeas
              hDirMeas
              ...
              hDirG
              hIndepGSwap
      _ = sphericalLaw d hDim1 (chiSqRadiusLaw d) := by
            simp [hRadG]

  have hMapEq :
      pushforward (Subtype.val : VecNZ d → Vec d) Q measurable_subtype_coe
        = pushforward (Subtype.val : VecNZ d → Vec d) (gaussianNZ d hDim1) measurable_subtype_coe := by
    calc
      pushforward (Subtype.val : VecNZ d → Vec d) Q measurable_subtype_coe
          = sphericalLaw d hDim1 (pushforward (radiusSq (d := d)) Q hRadMeas) := hPushQ
      _ = sphericalLaw d hDim1 (chiSqRadiusLaw d) := by
            simp [hRadQ]
      _ = pushforward (Subtype.val : VecNZ d → Vec d) (gaussianNZ d hDim1) measurable_subtype_coe := by
            symm
            exact hPushG
  ...
  apply Subtype.ext
  change (Q : Measure (VecNZ d)) = (gaussianNZ d hDim1 : Measure (VecNZ d))
  have hMapMeasure :
      Measure.map (Subtype.val : VecNZ d → Vec d) (Q : Measure (VecNZ d))
        = Measure.map (Subtype.val : VecNZ d → Vec d) (gaussianNZ d hDim1 : Measure (VecNZ d)) := by
  ...
  calc
    ...
    _ = Measure.comap (Subtype.val : VecNZ d → Vec d)
          (Measure.map (Subtype.val : VecNZ d → Vec d) (gaussianNZ d hDim1 : Measure (VecNZ d))) := by
            rw [hMapMeasure]
    _ = (gaussianNZ d hDim1 : Measure (VecNZ d)) := by
