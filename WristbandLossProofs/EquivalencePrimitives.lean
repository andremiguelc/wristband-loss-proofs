...
/-- Squared radius defined on all of `Vec d` (`= 0` at the origin). Used to
    state the chi-squared axiom about the full Gaussian, so the axiom does not
    have to mention the derived `gaussianNZ`. -/
def radiusSqVec {d : ℕ} (z : Vec d) : NNReal := ⟨‖z‖ ^ 2, by positivity⟩

lemma measurable_radiusSqVec (d : ℕ) : Measurable (radiusSqVec (d := d)) := by
  unfold radiusSqVec
  refine Measurable.subtype_mk ?_
  simpa using (continuous_norm.pow 2).measurable

@[simp] lemma radiusSqVec_subtype_val {d : ℕ} (z : VecNZ d) :
    radiusSqVec z.1 = radiusSq z := rfl
...
/-- Rotate a nonzero vector via a linear isometric equivalence.
    The result is nonzero because isometries preserve norms. -/
def rotateVecNZ {d : ℕ} (O : (Vec d) ≃ₗᵢ[ℝ] Vec d) (z : VecNZ d) : VecNZ d := by
  refine ⟨O z.1, ?_⟩
  intro hz
  apply z.2
  have hnorm : ‖O z.1‖ = ‖z.1‖ := O.norm_map z.1
  rw [hz, norm_zero] at hnorm
  exact norm_eq_zero.mp hnorm.symm

/-- Measurability of nonzero-vector rotations. -/
lemma measurable_rotateVecNZ {d : ℕ} (O : (Vec d) ≃ₗᵢ[ℝ] Vec d) :
    Measurable (rotateVecNZ O) := by
  unfold rotateVecNZ
  refine Measurable.subtype_mk ?_
  exact (O.continuous.comp continuous_subtype_val).measurable
