...
/-- The shifted Gaussian image sum is summable over `ℤ` for `β > 0`.

    Dominates each shifted Gaussian term by the Jacobi-theta bound
    `exp(-π (T·n² - 2·S·|n|))` with `T = 4β/π > 0`, then applies
    `summable_pow_mul_jacobiTheta₂_term_bound` (with `k = 0`). -/
lemma gaussianImageSum_summable
    (β : ℝ) (hβ : 0 < β) (c : ℝ) :
  ...
  refine hboundSummable.of_nonneg_of_le (fun n => Real.exp_nonneg _) ?_
  intro n
...
/-- The summand in the Neumann kernel is summable over `ℤ` for `β > 0`.
    Direct consequence of `gaussianImageSum_summable` applied to `t-t'`
    and `t+t'`. -/
lemma kernelRadNeumann_summable
    ...
    Summable (fun n : ℤ =>
      ...
      Real.exp (-β * ((t : ℝ) + (t' : ℝ) - 2 * n) ^ 2)) :=
  (gaussianImageSum_summable β hβ ((t : ℝ) - (t' : ℝ))).add
