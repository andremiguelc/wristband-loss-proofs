...
Let \(u \in S^{d_1-1}\) and \(v \in S^{d_2-1}\) be uniform (each has fixed norm 1).  
Concatenate and renormalize:

\[
w = \frac{[u, v]}{\lVert [u, v] \rVert} = \frac{[u, v]}{\sqrt{2}}
\]

Now \(w\) lies on \(S^{d_1+d_2-1}\), but it’s **not uniform**—because in \(w\), the energy in each block is fixed:

\[
\lVert w_{\text{text}} \rVert^2 = \lVert w_{\text{weather}} \rVert^2 = \frac{1}{2}.
\]
...
On the sphere, \(u = x / \lVert x \rVert\). That single division couples every coordinate to every other coordinate via:

\[
\lVert x \rVert = \sqrt{\sum_i x_i^2}.
\]
...
\[
(\text{tweet text}, \text{weather}) \rightarrow \text{likes}
\]
...
- \(E_{\text{text}}(\text{tweet}) \rightarrow z_{\text{text}}\)  
- \(E_{\text{weather}}(\text{weather}) \rightarrow z_{\text{weather}}\)  

Concatenate:

\[
z = [z_{\text{text}}, z_{\text{weather}}]
\]
...
So if you truly achieve \(z \sim N(0, I)\), then the blocks are independent:

\[
z_{\text{text}} \perp z_{\text{weather}}
\]

(and each marginal is standard normal too).
...
Encode the tweet text once:

\[
z_{\text{text}} = E_{\text{text}}(\text{tweet})
\]

Replace weather with a controlled variable:  
just random draws \(\varepsilon \sim N(0, I)\) to simulate “random weather”.

Predict likes from \([z_{\text{text}}, \varepsilon]\) many times to estimate a distribution.
...
- **Moment matching (\(\mu, \Sigma\))**: fast, but only constrains first 2 moments. Lots of ugly distributions share mean/covariance.  
...
- The radius has a known distribution (chi; equivalently \(\lVert x \rVert^2\) is chi‑square)
...
### Map each sample \(x \rightarrow\) (direction \(u\), radial CDF \(t\))

Given a sample \(x \in \mathbb{R}^d\):
...
\[
t = \mathrm{gammainc}\left(\frac{d}{2}, \frac{s}{2}\right)
\]

Under the null hypothesis \(x \sim N(0, I)\):
...
\[
W_2^2 = \lVert \mu \rVert^2 + \sum_i (\sqrt{\lambda_i} - 1)^2
\]
...
## The subtle part: why we need 3-image reflection on \(t \in [0, 1]\)
...
- sample many batches from true \(N(0, I)\) for your \((\text{batch\_size}, d)\)  
...
\[
\text{logit}(q, k) = \langle q, k \rangle - \frac{1}{2} \lVert k \rVert^2
\]

(plus learned scaling/normalization)
